        ...
        <div id="main">
            <div id="box_border">
                <ul id="fang_type">
                    ...
                    <li style="background-color: #000EFF;">可入住</li>
                    <li style="background-color: #b51d1a;">已入住</li>
					...
					<c:forEach var="i" begin="1" end="6">
		                <tr>
							...
							<c:forEach items="${tiers[i-1]}" var="room" varStatus="n">
								<c:if test="${ room.state eq '可入住'}">	<!-- 根据房间状态来显示不同的背景色 -->
									<c:if test="${n.count le 3}">		<!-- 根据房间号显示不同的单元(一单元...) -->	   
					                    <td width="40"  style="background-color: #000EFF" unitname='1'>
					                        <a href="/xngzf/archives/fangyuanRentHistory.action?fyID=14458579642011" class="fy_table_td" style="color:#fff;">${room.roomid }</a>
					                    </td>
				                    </c:if>
				               ...
				               <c:if test="${ room.state eq '已入住'}">	<!-- 根据房间状态来显示不同的背景色 -->
									<c:if test="${n.count le 3}">
					                    <td width="40"  style="background-color: #b51d1a" unitname='1'>
					                        <a href="/xngzf/archives/fangyuanRentHistory.action?fyID=14458579642011" class="fy_table_td" style="color:#fff;">${room.roomid }</a>
					                    </td>
				                    </c:if>
				                    <c:if test="${n.count gt 3 and n.count le 6}">
				               ...
				               <c:if test="${ room.state eq '钟点房'}">	<!-- 根据房间状态来显示不同的背景色 -->
									<c:if test="${n.count le 3}">
